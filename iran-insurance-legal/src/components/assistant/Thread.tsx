...
export function MessageView({ m, origin = 'assistant', compact }: { m: Message; origin?: 'home' | 'assistant'; compact?: boolean }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="space-y-2.5">
      ...
      {m.text && (
        <Markdown text={m.text} className={cn('text-[13.5px] leading-7 text-white/80', m.streaming && 'after:ms-0.5 after:inline-block after:h-3.5 after:w-1.5 after:animate-pulse after:rounded-sm after:bg-white/40 after:align-middle')} />
      )}
      {m.error && (
